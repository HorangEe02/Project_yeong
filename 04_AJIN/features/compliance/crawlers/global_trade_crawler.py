...
# 관세청 UNI-PASS OpenAPI — 관세율 조회 (HS code 기반)
UNIPASS_TARIFF_URL = "https://unipass.customs.go.kr:38010/ext/rest/trrtQry/retrieveTrrt"
# 자동차 부품 핵심 HS 8708 (Hyundai/Kia 공급망 핵심)
TARGET_HS_CODES = ["8708"]

_REFERENCE_URLS = {
    "US-IRA-001": "https://www.irs.gov/credits-deductions/clean-vehicle-credit-seller-or-dealer-requirements",
    "US-TSCA-001": "https://www.epa.gov/chemicals-under-tsca",
    "US-UFLPA-001": "https://www.cbp.gov/trade/forced-labor/UFLPA",
    "CN-REACH-001": "https://english.mee.gov.cn/Resources/laws/regulations/",
    "CN-NEV-001": "https://www.miit.gov.cn/",
    "US-S301-001": "https://ustr.gov/issue-areas/enforcement/section-301-investigations/tariff-actions",
    "US-USMCA-001": "https://ustr.gov/trade-agreements/free-trade-agreements/united-states-mexico-canada-agreement",
    "FTA-KORUS-001": "https://www.fta.go.kr/us/",
    "FTA-KOREU-001": "https://www.fta.go.kr/eu/",
}
...
class GlobalTradeCrawler(BaseCrawler):
    """글로벌 통상규제 크롤러 — 관세청 UNI-PASS OpenAPI + 큐레이트.

    CUSTOMS_API_KEY 가 설정돼 있으면 HS 8708(자동차 부품) 관세율을 라이브로
    조회해 큐레이트 항목 중 관세 영향이 있는 규제(`tariff_impact` 포함)에
    `live_tariff_kr_pct` 필드로 부착한다 (live 모드).
    미설정 또는 API 실패 시 큐레이트 _MASTER_DATA 만 반환 (curated 모드).
    """

    crawler_name = "global_trade"
    display_name = "글로벌 통상규제"
    doc_type = "Trade"
    legacy_filename = "global_trade.json"
    id_key = "regulation_id"
    name_key = "name_ko"
    status_key = "ajin_compliance_status"
    source_label = "unipass.customs.go.kr OpenAPI + 큐레이트"
    ...
    def _credentials_ready(self) -> bool:
        ...
        return bool(CUSTOMS_API_KEY)

    def _fetch_live(self) -> list[dict]:
        """관세청 UNI-PASS HS 관세율 조회 + curated 머지.

        F9 — `_http.fetch(use_cache=True)` 로 ETag/Last-Modified 자동 첨부.
        """
        from config import CUSTOMS_API_KEY

        live_rates: dict[str, str] = {}
        for hs in TARGET_HS_CODES:
            try:
                live_rates[hs] = self._lookup_tariff(hs, CUSTOMS_API_KEY)
            except Exception as e:
                logger.debug("HS %s 관세율 조회 실패: %s", hs, e)

        if not live_rates:
            raise RuntimeError("UNI-PASS 관세율 조회 실패 (API 키 무효 가능)")

        merged: list[dict] = []
        for entry in _MASTER_DATA:
            base = dict(entry)
            base.setdefault("reference_url", _REFERENCE_URLS.get(str(base.get("regulation_id")), ""))
            if base.get("tariff_impact"):
                base["live_tariff_lookup"] = live_rates
                base["live_meta_source"] = "unipass.customs.go.kr"
            merged.append(base)
        return merged

    def _lookup_tariff(self, hs_code: str, api_key: str) -> str:
        """관세청 trrtQry — XML 응답에서 trrt(관세율) 추출 (F9 캐시 통합)."""
        from features.compliance.infra._http import fetch
        url = f"{UNIPASS_TARIFF_URL}?crkyCn={api_key}&hsSgn={hs_code}"
        resp = fetch(url, use_cache=True)
        # http_meta 갱신 (BaseCrawler audit 적재용)
        ...
        body = resp.text
        m = re.search(r"<trrt>(?P<rate>[^<]+)</trrt>", body)
        if not m:
            raise RuntimeError(f"<trrt> 태그 없음: {body[:200]}")
    ...
    def load(self) -> list[GlobalTradeRegulation]:
        ...
        with open(path, encoding="utf-8") as f:
            data = json.load(f)
        items = data.get("data") or data.get("regulations") or []
        regs: list[GlobalTradeRegulation] = []
        for item in items:
            clean = {
                k: v for k, v in item.items()
                if k not in ("live_meta_source", "live_tariff_lookup")
            }
            try:
                regs.append(GlobalTradeRegulation(**clean))
            except TypeError as e:
