...
UNECE_REGULATIONS_URL = (
    "https://unece.org/transport/standards/transport/vehicle-regulations-wp29/regulations"
)
UN_GTR_20_ODS_URL = "https://docs.un.org/ECE/TRANS/180/Add.20"
UN_R100_STATUS_ODS_URL = "https://docs.un.org/ECE/TRANS/WP.29/343/REV.30/ADD.2"
UN_R100_DOCUMENTS_PDF_URL = (
    "https://documents.un.org/api/symbol/access"
    "?l=en&s=ECE%2FTRANS%2FWP.29%2F343%2FREV.30%2FADD.2&t=pdf"
)
...
_MASTER_DATA: list[dict] = [
    # ── UN GTR 20 ──
    {
        ...
        "reference_url": UN_GTR_20_ODS_URL,
    },
    # ── UN R100 ──
    {
        "standard_id": "UN-R100",
        ...
        "reference_url": UN_R100_STATUS_ODS_URL,
    },
    # ── IEC 62660 시리즈 ──
    {
...
class EVBatteryCrawler(BaseCrawler):
    """EV 배터리 안전 규격 크롤러 — UNECE WP.29 R100 라이브 + 큐레이트 임팩트 분석.

    UNECE 차량 규정 인덱스 페이지를 BeautifulSoup 으로 파싱해 R100 의 최신
    revision · published_date · pdf_url 을 추출하고, 큐레이트 _MASTER_DATA 의
    UN-R100 항목에 덮어쓴다 (live 모드). HTML 파싱 실패 시 큐레이트만 반환
    (curated 모드).

    UN GTR 20, IEC, SAE, GB/T 등 다른 규격은 본 Phase 1 범위 밖 — 큐레이트
    그대로 통과한다 (라이선스/페이로드 이슈로 자동 크롤이 어려움).
    """

    crawler_name = "ev_battery"
    display_name = "EV 배터리 규격"
    doc_type = "EV"
    legacy_filename = "ev_battery.json"
    id_key = "standard_id"
    name_key = "name_ko"
    status_key = "ajin_compliance_status"
    source_label = "unece.org/wp29 + UN ODS (R100 live) + curated master"
    ...
    def _fetch_live(self) -> list[dict]:
        """UNECE 인덱스 또는 UN ODS에서 R100 메타 추출."""
        from features.compliance.infra._http import fetch_text
        from bs4 import BeautifulSoup  # type: ignore[import-not-found]

        merged: list[dict] = [dict(e) for e in _MASTER_DATA]
        try:
            html = fetch_text(UNECE_REGULATIONS_URL)
        except Exception as e:
            direct_error = e
        else:
            meta = self._parse_r100_metadata(html, BeautifulSoup)
            if not meta:
                raise RuntimeError("UNECE 페이지에서 R100 메타를 찾지 못함")
            return self._apply_r100_metadata(merged, meta, "unece.org/wp29")

        try:
            ods_text = fetch_text(UN_R100_STATUS_ODS_URL)
            if not ods_text.strip():
                raise RuntimeError("UN ODS R100 status page returned an empty response")
        except Exception as ods_error:
            raise RuntimeError(
                "UNECE 인덱스 및 UN ODS R100 조회 실패: "
                f"direct={type(direct_error).__name__}: {direct_error}; "
                f"ods={type(ods_error).__name__}: {ods_error}"
            ) from ods_error

        meta = {
            "revision": "REV.30 ADD.2",
            "pdf_url": UN_R100_DOCUMENTS_PDF_URL,
            "published_date": "",
        }
        return self._apply_r100_metadata(merged, meta, "docs.un.org ODS")

    @staticmethod
    def _apply_r100_metadata(
        merged: list[dict],
        meta: dict[str, str],
        source: str,
    ) -> list[dict]:
        """Apply official R100 metadata to the curated UN-R100 entry."""
        for item in merged:
            if item.get("standard_id") == "UN-R100":
                # 라이브에서 받은 신선한 메타데이터를 덮어쓰되, 빈 값은 큐레이트 보존
                if meta.get("revision"):
                    item["version"] = meta["revision"]
                if meta.get("published_date"):
                    item["effective_date"] = meta["published_date"]
                if meta.get("pdf_url"):
                    item["reference_url"] = meta["pdf_url"]
                item["live_meta_source"] = source
                break
        return merged

    @staticmethod
    def _parse_r100_metadata(html: str, BeautifulSoup) -> dict | None:
        """UNECE WP.29 regulations 페이지에서 R100 행을 찾아 메타 추출."""
        soup = BeautifulSoup(html, "lxml")
        for a in soup.find_all("a", href=True):
            href = a["href"]
            text = (a.get_text() or "").strip()
            if "R100" not in href and "R100" not in text:
                continue
            # 형식 예: ".../R100r3e.pdf" → revision = "r3"
            m = re.search(r"R100(r\d+)?[a-z]?\.pdf", href, re.IGNORECASE)
            if not m:
                continue
            revision = (m.group(1) or "").lower() or "r0"
            pdf_url = href if href.startswith("http") else f"https://unece.org{href}"
            return {
                "revision": revision.upper(),
                "pdf_url": pdf_url,
                "published_date": "",  # PDF metadata 추출은 Phase 1 범위 밖
            }
        return None

    def _curated_fallback(self) -> list[dict]:
        return [dict(e) for e in _MASTER_DATA]
    ...
    def crawl(self) -> EVBatteryCrawlResult:
        ...
        _known = {f.name for f in _dc_fields(EVBatteryStandard)}
        for item in items:
            clean = {k: v for k, v in item.items() if k in _known and k != "crawled_at"}
            try:
                standards.append(EVBatteryStandard(**clean, crawled_at=now))
        ...
        today = now[:10]
        action_needed = sum(
            1 for s in standards if s.ajin_compliance_status in ("미충족", "부분충족")
        )
        upcoming = sum(
            1 for s in standards if s.transition_deadline and s.transition_deadline >= today
        )

        ds = EVBatteryCrawlResult(
            standards=standards,
            crawled_at=now,
        ...
        ds.source_type = result_dict.get("source_type", "curated")  # type: ignore[attr-defined]
        return ds
    ...
    def load(self) -> list[EVBatteryStandard]:
        ...
        with open(path, encoding="utf-8") as f:
            data = json.load(f)
        items = data.get("data") or data.get("standards") or []
        out: list[EVBatteryStandard] = []
        for item in items:
            clean = {k: v for k, v in item.items() if k != "live_meta_source"}
            try:
                out.append(EVBatteryStandard(**clean))
            except TypeError:
                continue
        self._standards = out
        return out
