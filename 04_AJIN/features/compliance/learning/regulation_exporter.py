...
def _build_plant_detail_section(doc_type: str) -> list[str]:
    ...
    try:
        from features.compliance.infra.plant_regulation_mapper import get_applicable_plants
    except ImportError:
        return []
...
def regulations_to_markdown(
    ...
    v4.x: include_enrichment=True 일 때 regulation_changes 테이블에서
          과거/현재 diff + 사내 영향 분석을 항목 뒤에 추가 (Issue 3).
    """
    lines = []
    ...
    # 개별 항목
    reg_type_for_query = regulation_type or doc_type.lower()
    for i, item in enumerate(items, 1):
        ...
        # v4.x — 항목별 변경 이력 + 사내 영향 (Issue 3)
        if include_enrichment and reg_type_for_query:
            from features.compliance.learning.change_enrichment import (
                build_change_history_section,
                build_impact_section,
            )
            item_id = str(item.get("id") or item.get("name") or item.get("title") or "")
            lines.extend(
                build_change_history_section(reg_type_for_query, item_id, limit=3)
            )
            lines.extend(build_impact_section(reg_type_for_query, item_id))
...
def export_regulations_docx(
    ...
    try:
        ...
        md = regulations_to_markdown(
            items, doc_type, display_name, crawled_at,
