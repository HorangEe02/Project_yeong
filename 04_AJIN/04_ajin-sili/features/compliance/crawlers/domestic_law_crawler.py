...
class DomesticLawCrawler(BaseCrawler):
    """국내 법규 통합 크롤러 — open.law.go.kr OpenAPI + 큐레이트 임팩트 분석.

    LAW_GO_KR_OC 환경변수가 설정돼 있으면 법제처 OpenAPI 로 각 법령의 최신
    last_amended/시행일을 받아와 큐레이트 데이터에 덮어쓴다 (live 모드).
    미설정 또는 API 실패 시 큐레이트 _DOMESTIC_LAWS 만 반환 (curated 모드).
    """

    crawler_name = "domestic_law"
    display_name = "국내 법규"
    doc_type = "DomesticLaw"
    legacy_filename = "domestic_laws.json"
    id_key = "law_id"
    name_key = "name"
    status_key = "compliance_status"
    source_label = "law.go.kr OpenAPI + 큐레이트"

    _LAW_API_URL = "https://www.law.go.kr/DRF/lawSearch.do"
    ...
    def _credentials_ready(self) -> bool:
        from config import LAW_GO_KR_OC  # 지연 import — config.py 순환 참조 회피
        return bool(LAW_GO_KR_OC)

    def _fetch_live(self) -> list[dict]:
        """각 큐레이트 법령에 대해 OpenAPI 로 last_amended 갱신 + 신규 항목 머지.

        F8/F9 — `self._cached_fetch_json` 으로 ETag/Last-Modified conditional GET.
        법제처는 보통 ETag 미제공이라 304 hit 는 드물지만, http_meta 는 항상 적재된다.
        """
        from config import LAW_GO_KR_OC

        merged: list[dict] = []
        for entry in _DOMESTIC_LAWS:
            base = dict(entry)
            try:
                api_meta = self._lookup_law(base["name"], LAW_GO_KR_OC,
                                            self._cached_fetch_json)
                if api_meta:
                    base["last_amended"] = api_meta.get("last_amended") or base.get("last_amended", "")
                    base["live_meta_source"] = "law.go.kr"
                    base["live_effective_date"] = api_meta.get("effective_date") or ""
            except Exception as e:
                logger.debug("법령 메타 조회 실패 (%s): %s", base.get("name"), e)
            merged.append(base)
        return merged

    def _lookup_law(self, query: str, oc: str, fetch_json) -> dict | None:
        """법제처 lawSearch API — 가장 최신 본법 1건의 last_amended/effective_date 반환."""
        params = {
            "OC": oc,
            "target": "law",
            "type": "JSON",
        ...
        url = self._LAW_API_URL + "?" + "&".join(f"{k}={v}" for k, v in params.items())
        data = fetch_json(url)
        if data is None:
            # 304 — 캐시된 메타 그대로 사용. last_lookup 결과는 다음 호출 사이클에 갱신.
            return None
        # 응답 구조: {"LawSearch": {"law": [{"개정일자": "20240101", "시행일자": "20240701", ...}]}}
        try:
            laws = data.get("LawSearch", {}).get("law") or []
            if not laws:
                return None
            head = laws[0] if isinstance(laws, list) else laws
            return {
                "last_amended": _format_yyyymmdd(head.get("개정일자")),
                "effective_date": _format_yyyymmdd(head.get("시행일자")),
            }
        except (AttributeError, KeyError, IndexError):
            return None

    def _curated_fallback(self) -> list[dict]:
        return [dict(e) for e in _DOMESTIC_LAWS]

    # ── 공개 API: 백워드 호환 ──
    def crawl(self) -> DomesticLawCrawlResult:
        """기존 호출자(`features/compliance/__init__.py`) 호환 — dataclass 반환."""
        ...
        laws: list[DomesticLaw] = []
        # dataclass 필드만 통과 — 라이브 오버레이가 추가한 외부 키는 무시
        from dataclasses import fields as _dc_fields
        _known_fields = {f.name for f in _dc_fields(DomesticLaw)}
        for item in items:
            try:
                clean = {k: v for k, v in item.items() if k in _known_fields and k != "crawled_at"}
                laws.append(DomesticLaw(**clean, crawled_at=now))
            except TypeError as e:
                errors.append(f"파싱 오류 ({item.get('law_id', '?')}): {e}")
        self._laws = laws

        ds = DomesticLawCrawlResult(
            laws=laws,
            crawled_at=now,
        ...
        # source_type 을 dataclass 객체에 동적 부착 — _summarize_crawl 에서 getattr 로 픽업
    ...
    def load(self) -> list[DomesticLaw]:
        ...
        with open(path, encoding="utf-8") as f:
            data = json.load(f)
        items = data.get("data") or data.get("laws") or []
        out: list[DomesticLaw] = []
        for item in items:
            clean = {k: v for k, v in item.items() if k != "live_meta_source"}
            try:
                out.append(DomesticLaw(**clean))
            except TypeError:
                continue
        return out
...
def _format_yyyymmdd(s: Any) -> str:
    """법제처 응답의 '20240101' 형태를 '2024-01-01' 로 정규화."""
    if not s:
        return ""
    s = str(s).strip()
