...
class MSDSCrawler:
    ...
    def crawl(self) -> MSDSCrawlResult:
        ...
        for item in _MSDS_RECORDS:
            try:
                ...
                if not normalized.get("reference_url"):
                    normalized["reference_url"] = self.NCIS_BASE_URL
                records.append(MSDSRecord(**normalized))
        ...
        result.source_reason = (  # type: ignore[attr-defined]
            "MSDS/SVHC baseline is curated from official NCIS, ECHA, and supplier sources; "
            "live ECHA reachability is checked by the release verifier."
        )
    ...
    def _save(self, result: MSDSCrawlResult):
        ...
        data = {
            ...
            "chemicals": [asdict(r) for r in result.records],
            "svhc_updates": [asdict(s) for s in result.svhc_updates],
            "data": [asdict(r) for r in result.records] + [asdict(s) for s in result.svhc_updates],
