...
from features.compliance.infra.impact_analyzer import ImpactReport
...
class AlertGenerator:
    ...
    def generate(self, report: ImpactReport) -> Alert:
        ...
        icon = SEVERITY_TAGS.get(report.severity, "[INFO]")
    ...
    def _build_summary(self, report: ImpactReport) -> str:
        tag = SEVERITY_TAGS.get(report.severity, "[INFO]")
        label = SEVERITY_LABELS.get(report.severity, "정보")
        return (
            f"{tag} [{label}] {report.title}\n"
