...
// 토큰별 깜빡임을 방지.
//
// v3.7 추가: HR/행정 문서 (휴가/출장/사직/인사발령) 의 plain header (수신/발신/
// 제목/본문/결재란) 분리, bracket+bullet 패턴, 자주 쓰이는 한자 → 한글 치환.

const KOREAN_TERMINATIONS = '다요까오네지함음됨임'; // -다/요/까/오/네/지/함/음/됨/임

// HR/행정 문서에서 LLM 이 자주 출력하는 한자 → 한글 치환 사전.
// 한국어 LLM 도 학습 데이터의 영향으로 일부 한자 그대로 출력하는 케이스가 있어 보정.
...
// 행정 문서의 plain header 단어 (마크다운 ** 미사용 시) — 줄 시작 아니면 \n\n 삽입.
const PLAIN_HEADERS = ['수신', '발신', '제목', '본문', '결재란'];
...
export function formatDraftOutput(text: string): string {
  ...
  // 1.5. plain header (수신/발신/제목/본문/결재란) — 줄 시작 아니면 \n\n 삽입.
  //      LLM 이 마크다운 ** 없이 한국어 행정 헤더를 한 줄로 이어 출력하는 경우 대응.
  //      예) "...담당자수신: 인사팀장" → "...담당자\n\n수신: 인사팀장"
  //      예) "...[성함]제목: 휴가" → "...[성함]\n\n제목: 휴가"
  //      tab/space/줄바꿈/대괄호 닫는 ] 뒤 모두 매칭. 단, 이미 새 줄이면 미적용 (룩비하인드).
  ...
  // 5.5. bracket + bullet — "[휴가 종류]- 기간:" 처럼 공백 없이 ] 뒤 "- " 가 오는 케이스.
  //      한국어 행정 폼의 placeholder 다음 bullet 이 자주 이 패턴.
  s = s.replace(/(\])\s*(- )/g, '$1\n$2');
  ...
  // 6.5. 한자 → 한글 치환 (HR/행정 문서에서 자주 등장).
  //      LLM 학습 데이터의 영향으로 不在/出張/結裁 같은 한자가 그대로 출력되는 경우 보정.
  for (const [hanja, hangul] of Object.entries(HANJA_MAP)) {
    s = s.split(hanja).join(hangul);
  }
