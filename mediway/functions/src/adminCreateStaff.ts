...
export async function handleCreateStaff(
  ...
  // 5.5 Custom Claims 주입 — staff 계정은 생성 즉시 hospitalId claim이 필수.
  //     실패 시 staff 첫 로그인에서 RTDB 401이 확정되므로 throw로 전파해 호출자가 롤백 판단.
  try {
    await injectClaimsForUid(user.uid, 'adminCreateStaff', true);
  } catch (err) {
    console.error('[adminCreateStaff] claim 주입 실패 — 생성된 Auth+RTDB 정리', err);
    try {
      await admin.auth().deleteUser(user.uid);
      await db.ref(`users/${user.uid}`).remove();
    } catch (cleanupErr) {
      console.error('[adminCreateStaff] 롤백 실패', cleanupErr);
    }
    throw new HttpsError(
      'internal',
      'Custom Claims 주입에 실패해 계정 생성을 취소했습니다',
    );
  }
